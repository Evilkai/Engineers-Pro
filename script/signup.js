function validateRegno() {
  ...
  $.ajax({
    ...
    success: function(response) {
      var { status, result } = JSON.parse(response);
      if (status == "invalid") {
        swal({
        ...
        var { fname, ffather, fmother, fregno } = result;
        if (univcode == "001") {
          $("#aperinfo")
            .html(`<table class="table no-border" style="color:#000;font-weight:600;font-size:14px">
...
function validateInfo() {
  ...
  $.ajax({
    ...
    success: function(response) {
      var { status, result } = JSON.parse(response);
      if (status == "invalid") {
        var { fmobexist, femailexist, fregexist } = result;
        var message;
        var id;
        if (fmobexist >= "1") {
